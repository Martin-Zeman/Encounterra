...
def process_index_results(job_id, index):
    # Construct the object key prefix for the index
    index_prefix = f"{job_id}/{index}/"

    # List all objects under the index prefix
    objects = s3.list_objects_v2(Bucket=bucket_name, Prefix=index_prefix)

    # Initialize statistics for blue and red teams
    statistics = {color: {stat.name: 0 for stat in Statistics} for color in ['BLUE', 'RED']}

    # Iterate over the objects and aggregate the results
    for obj in objects.get('Contents', []):
        object_key = obj['Key']
...
def handler(event, context):
    ...
    for index in range(iterations):
        index_key = f"{job_id}/{index}/simulation_results.json"
        try:
            ...
            statistics['BLUE'][Statistics.AT_LEAST_THREE_DIED.name] += result.get('blue_at_least_three_died', 0)
            statistics['RED'][Statistics.AT_LEAST_THREE_DIED.name] += result.get('red_at_least_three_died', 0)
            s3.delete_object(Bucket=bucket_name, Key=index_key)
        except ClientError as e:
            logger.error(f"Failed to retrieve or process result {index_key}: {e}")
            exit(1)
