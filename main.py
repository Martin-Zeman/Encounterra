...
if __name__ == '__main__':
    # Logging disabled for benchmarking (no stream handler, level above CRITICAL).
    CustomLogger(logging.CRITICAL, False)
    session = Session()
    # Equivalent matchup to c++/main.cpp: 1 Bugbear (BLUE) vs 3 Goblins (RED).
    ...
    # session.add_combatant(DragonclawCultist, Teams.Color.RED)
    # session.add_combatant(DragonclawCultist, Teams.Color.RED)
    # session.add_combatant(EvilMage, Teams.Color.RED)
    ...
    # session.add_combatant(Hobgoblin, Teams.Color.BLUE)
    # session.add_combatant(Hobgoblin, Teams.Color.RED)
    # session.add_combatant(Hobgoblin, Teams.Color.RED)
    # session.add_combatant(Goblin, Teams.Color.BLUE)
    # session.add_combatant(OathOfVengeancePaladin4Lvl, Teams.Color.BLUE)
    ...
    # session.add_combatant(TotemBarbarian5Lvl, Teams.Color.RED)
    # session.add_combatant(BattlemasterFighter5Lvl, Teams.Color.BLUE)
    # session.add_combatant(BattlemasterFighter4Lvl, Teams.Color.BLUE)
    # session.add_combatant(Zombie, Teams.Color.BLUE)
    # session.add_combatant(Hobgoblin, Teams.Color.RED)
    # session.add_combatant(SaberToothedTiger, Teams.Color.BLUE)
    # session.add_combatant(HillGiant, Teams.Color.RED)
    # session.add_combatant(Owlbear, Teams.Color.RED)
    ...
    # Blank map to mirror c++/main.cpp (no terrain or obstacles).
    session.place_terrain_and_obstacles(Session.MapType.BLANK.value)
    session.simulate(parallel=False)
    print("--- Python simulation (10 iterations) took {:.1f} seconds ---".format((time.time() - start_time)))
