...
if __name__ == '__main__':
    ...
    battle_map.set_character_coordinates(cyanwrath, random.randint(1, 15), random.randint(9, 15))
    battle_map.set_character_coordinates(rena, random.randint(1, 15), random.randint(1, 8))
    combat_manager = CombatManager(combatants, teams)
    round_manager = RoundManager(combatants, teams, battle_map, combat_manager)
    # battle_map.build_adjacency_matrix()
    battle_map.place_circular_element((random.randint(1, 15), random.randint(1, 15)), Map.DIFFICULT_TERRAIN, random.randint(1, 2))
    battle_map.place_circular_element((random.randint(1, 15), random.randint(1, 15)), Map.DIFFICULT_TERRAIN, random.randint(1, 2))
    battle_map.place_circular_element((random.randint(1, 15), random.randint(1, 15)), Map.DIFFICULT_TERRAIN, random.randint(1, 2))
    battle_map.build_adjacency_matrix()
