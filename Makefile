...
DOCKER_BATCH_CONTAINER = encounterra-batch_core
...
docker.build_batch:
	...
	aws ecr get-login-password --region eu-west-1 | docker login --username AWS --password-stdin ${DOCKER_SANDBOX_REMOTE}
	docker build -t ${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED} --build-arg APP_VERSION=${GIT_COMMIT} --network host -f Dockerfile_batch .
	docker tag ${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED} ${DOCKER_BATCH_CONTAINER}:latest
	@if [ -z "${GIT_STATUS}" ]; then \
        docker tag ${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED} ${DOCKER_BATCH_CONTAINER}:${GIT_COMMIT}; \
        docker tag ${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED} ${DOCKER_SANDBOX_REMOTE}/${DOCKER_BATCH_CONTAINER}:${GIT_COMMIT}; \
    fi
	@echo ""
	@echo "*"
	@echo "*"
	@echo "* Docker container built"
	@echo "*"
	@echo "*  tagged: ${FONT_BOLD}${DOCKER_BATCH_CONTAINER}:latest${FONT_NORMAL}"
	@echo "*  tagged: ${FONT_BOLD}${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED}${FONT_NORMAL}"
	@if [ -z "${GIT_STATUS}" ]; then \
		docker tag ${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED} ${DOCKER_BATCH_CONTAINER}:${GIT_COMMIT}; \
		docker tag ${DOCKER_BATCH_CONTAINER}:${GIT_BRANCH_SANITIZED} ${DOCKER_SANDBOX_REMOTE}/${DOCKER_BATCH_CONTAINER}:${GIT_COMMIT}; \
	else \
...
docker.push_batch:
	...
	docker push ${DOCKER_SANDBOX_REMOTE}/${DOCKER_BATCH_CONTAINER}:${GIT_COMMIT}
	...
	@echo "* Container pushed"
	@echo "*"
	@echo "*  pushed: ${DOCKER_SANDBOX_REMOTE}/${DOCKER_BATCH_CONTAINER}:${GIT_COMMIT}"
	@echo "* "
	@if [ -z "${GIT_STATUS}" ]; then \
        echo "* short commit nr: ${GIT_COMMIT}"; \
    fi
