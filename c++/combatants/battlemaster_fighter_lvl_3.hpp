#pragma once

#include "core/combatant.hpp"
#include <string_view>

namespace enc
{
  class BattlemasterFighterLvl3 : public Combatant
  {
  public:
    BattlemasterFighterLvl3(int num);
    BattlemasterFighterLvl3(const std::string &name);

    int getClassId() const override { return _classId; }

    static constexpr int getStaticClassId() { return _classId; }
    static constexpr std::string_view getClassName() { return _className; }

  private:
    static constexpr std::string_view _className = "Battlemaster Fighter LVL 3";
    static constexpr int _classLevel = 3;
    static constexpr int _classId = Combatant::generateClassId(_className, Fighter::BATTLE_MASTER, _classLevel);
  };
}
