...
**Encounterra** is a combat encounter simulator for Dungeons and Dragons 5th 2024 Edition. It allows DMs to quickly create and simulate encounters with various monsters, NPCs, and player characters, providing detailed combat logs and statistics.
...
It is only listed for an easy look-up of the original implementation. Python is mostly 2014 version of the rules.
...
The C++ implementation is a work in progress. The puts large emphasis on performance optimization. For matrix operations, it uses the Blaze library. GTest is the testing framework of choice. The C++ code is organized in a similar way to the Python code, with separate directories for different components of the simulation. The main simulation loop and combatant logic are still being developed, but the core data structures and some of the action logic have been implemented.
