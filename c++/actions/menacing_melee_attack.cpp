...
namespace enc
{
  MenacingMeleeAttackFactory::MenacingMeleeAttackFactory(const std::string &name, Combatant *combatant, int toHit, std::vector<Die> dmgDice,
                                                         int dmgBonus, DamageType dmgType, int attackRange, int critRange, Uses &&ammo, OnHit *onHit,
                                                         std::vector<DmgDieWithType> extraDmg, bool usesDex, bool twoHanded, Die toHitBonusDie)
      : MeleeAttackFactory(name, combatant, toHit, dmgDice, dmgBonus, dmgType, attackRange, critRange, std::move(ammo), onHit, extraDmg, usesDex,
