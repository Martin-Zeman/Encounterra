...
namespace enc
{
  ...
  std::vector<Actoid *>
  DefaultActionPlanStrategy::calculateActionPlan(const blaze::DynamicVector<int> &distances, const blaze::DynamicMatrix<Coord> &shortestPaths)
  {
    ...
    if(fsm.getNumStates() == 2U)
      { // TODO: Check if this is the right proxy for an empty FSM
        std::vector<Actoid *> movement;
    ...
    auto [bestSequence, maxThreat, msTransitionPaths]
      = findBestSequence(_combatant, fsm, transitionToEligibleCoords, movementTransToCoordAndType, distances, shortestPaths);
  ...
  {
    std::vector<Actoid *> bestSequence;
  ...
  std::vector<Actoid *>
  DefaultActionPlanStrategy::extractMovement(const blaze::DynamicVector<int> &distances, const blaze::DynamicMatrix<Coord> &shortestPaths,
        ...
        if(auto *movement = dynamic_cast<MovementIncrement *>(action))
          {
            ...
            auto pathOpt = battleMap.getPathToCoord(_combatant, targetCoord, distances, shortestPaths, true);
            if(pathOpt)
              {
