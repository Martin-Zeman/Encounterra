...
class NeedleBlight(Combatant):
    ...
    def __init__(self, num_or_name=1):
        ...
        self.claws = self.add_ability(Action.MELEE_ATTACK,  name="Claws", combatant=self, to_hit=3, dmg_dice=((2, 4),), dmg_bonus=1, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1)
        self.needles = self.add_ability(Action.RANGED_ATTACK, name="Needles", combatant=self, to_hit=4, dmg_dice=((2, 6),), dmg_bonus=1, dmg_type=DamageType.Piercing, attack_range=12, crit_range=1, ammo=Uses(math.inf, ResourceRefreshType.NEVER))
        self.add_ability(Reaction.REACTION_ATTACK,  name="Claws", combatant=self, to_hit=3, dmg_dice=((2, 4),), dmg_bonus=1, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1)
