...
class BugbearChief(Combatant):
    ...
    def __init__(self, num_or_name=1):
        ...
        self.morningstar_attack = self.add_ability(Action.MELEE_ATTACK,  name="Morningstar", combatant=self, to_hit=5, dmg_dice=((2, 8),), dmg_bonus=3, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1)
        self.javelin_attack = self.add_ability(Action.RANGED_ATTACK,  name="Javelin", combatant=self, to_hit=4, dmg_dice=((1, 6),), dmg_bonus=3, dmg_type=DamageType.Piercing, attack_range=24, crit_range=1, ammo=Uses(1, ResourceRefreshType.NEVER), uses_dex=False)
        self.add_ability(Reaction.REACTION_ATTACK,  name="Morningstar", combatant=self, to_hit=5, dmg_dice=((2, 8),), dmg_bonus=3, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1)
