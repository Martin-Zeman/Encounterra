...
class FireGiant(Combatant):
    ...
    def __init__(self, num_or_name=1):
        super().__init__(num_or_name, hp=162, ac=18, init_bonus=-1, spell_to_hit=0, speed=40, immunities={DamageType.Fire}, resistances=set(), dc=0)
        self.size = Size.HUGE
        self.sword = self.add_ability(Action.MELEE_ATTACK,  name="Greatsword", combatant=self, to_hit=11, dmg_dice="6d6", dmg_bonus=7, dmg_type=DamageType.Slashing, attack_range=3)
        self.rock = self.add_ability(Action.RANGED_ATTACK, name="Rock", combatant=self, to_hit=11, dmg_dice="4d10", dmg_bonus=7,
                                            dmg_type=DamageType.Bludgeoning, attack_range=48, crit_range=1, ammo=Uses(2, ResourceRefreshType.NEVER), uses_dex=False)
        self.add_ability(Reaction.REACTION_ATTACK,  name="Greatsword", combatant=self, to_hit=11, dmg_dice="6d6", dmg_bonus=7, dmg_type=DamageType.Slashing, attack_range=3)
        self.build_attack_fms()
        self.saving_throws[SavingThrow.STR] = 7
        self.saving_throws[SavingThrow.DEX] = 3
        self.saving_throws[SavingThrow.CON] = 10
