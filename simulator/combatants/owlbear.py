...
class Owlbear(Combatant):
    ...
    def __init__(self, num_or_name=1):
        super().__init__(num_or_name, hp=59, ac=13, init_bonus=1, spell_to_hit=0, speed=40, resistances=set(), dc=0)
        self.size = Size.LARGE
        self.beak = self.add_ability(Action.MELEE_ATTACK,  name="Beak", combatant=self, to_hit=7, dmg_dice=[(1, 10)], dmg_bonus=5, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1)
        self.claws = self.add_ability(Action.MELEE_ATTACK,  name="Claws", combatant=self, to_hit=7, dmg_dice=[(2, 8)], dmg_bonus=5, dmg_type=DamageType.Slashing, attack_range=1, crit_range=1)
        self.add_ability(Reaction.REACTION_ATTACK,  name="Claws", combatant=self, to_hit=7, dmg_dice=[(2, 8)], dmg_bonus=5, dmg_type=DamageType.Slashing, attack_range=1, crit_range=1)
