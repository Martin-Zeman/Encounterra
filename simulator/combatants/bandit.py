...
class Bandit(Combatant):
    ...
    def __init__(self, num_or_name=1):
        ...
        self.scimitar = self.add_ability(Action.MELEE_ATTACK,  name="Scimitar", combatant=self, to_hit=3, dmg_dice=[(1, 6)], dmg_bonus=1, dmg_type=DamageType.Slashing, attack_range=1, crit_range=1)
        self.light_crossbow = self.add_ability(Action.RANGED_ATTACK,  name="Light Crossbow", combatant=self, to_hit=4, dmg_dice=[(1, 8)], dmg_bonus=1, dmg_type=DamageType.Piercing, attack_range=64, crit_range=1, ammo=Uses(10, ResourceRefreshType.NEVER))
        self.add_ability(Reaction.REACTION_ATTACK,  name="Scimitar", combatant=self, to_hit=3, dmg_dice=[(1, 6)], dmg_bonus=1, dmg_type=DamageType.Slashing, attack_range=1, crit_range=1)
