...
class TotemBarbarian5Lvl(Combatant):
    ...
    name = "Totem Barbarian 5th LVL"
    cls = Class.BARBARIAN.PATH_OF_THE_TOTEM_WARRIOR
    level = 5
    id = Combatant.generate_unique_id(name, cls, level)

    def __init__(self, num_or_name=1):
        super().__init__(num_or_name, hp=61, ac=15, init_bonus=1, spell_to_hit=0, speed=40, resistances=set(), dc=15)
        self.axe = self.add_ability(Action.MELEE_ATTACK,  name="Two-handed axe", combatant=self, to_hit=7, dmg_dice=[(1, 12)], dmg_bonus=4, dmg_type=DamageType.Slashing, attack_range=1)
        self.javelin_attack = self.add_ability(Action.RANGED_ATTACK, name="Javelin", combatant=self, to_hit=7, dmg_dice=[(1, 6)], dmg_bonus=4, dmg_type=DamageType.Piercing, attack_range=24, crit_range=1, ammo=Uses(4, ResourceRefreshType.NEVER), uses_dex=False)
        self.add_ability(Reaction.REACTION_ATTACK,  name="Two-handed axe", combatant=self, to_hit=7, dmg_dice=[(1, 12)], dmg_bonus=4, dmg_type=DamageType.Slashing, attack_range=1)
        self.add_ability(BonusAction.TOTEM_RAGE)
        self.add_ability(Passive.DANGER_SENSE)
        self.add_ability(Passive.UNARMORED_DEFENSE)
        self.axe_recklessly = self.add_ability(Action.RECKLESS_ATTACK, name="Two-handed axe recklessly", combatant=self, to_hit=7, dmg_dice=[(1, 12)], dmg_bonus=4, dmg_type=DamageType.Slashing, attack_range=1)
