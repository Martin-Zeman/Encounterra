...
class HillGiant(Combatant):
    ...
    def __init__(self, num_or_name=1):
        ...
        self.size = Size.HUGE
        self.club = self.add_ability(Action.MELEE_ATTACK,  name="Greatclub", combatant=self, to_hit=8, dmg_dice="3d8", dmg_bonus=5, dmg_type=DamageType.Bludgeoning, attack_range=3)
        self.rock = self.add_ability(Action.RANGED_ATTACK, name="Rock", combatant=self, to_hit=8, dmg_dice="3d10", dmg_bonus=5,
                                            dmg_type=DamageType.Bludgeoning, attack_range=48, crit_range=1, ammo=Uses(2, ResourceRefreshType.NEVER), uses_dex=False)
        self.add_ability(Reaction.REACTION_ATTACK,  name="Greatclub", combatant=self, to_hit=8, dmg_dice="3d8", dmg_bonus=5, dmg_type=DamageType.Bludgeoning, attack_range=3)
