...
class WhiteDragonWyrmling(Combatant):
    ...
    def __init__(self, num_or_name=1):
        super().__init__(num_or_name, hp=32, ac=16, init_bonus=0, spell_to_hit=0, speed=60, immunities={DamageType.Cold}, resistances=set(), dc=0)
        self.bite = self.add_ability(Action.MELEE_ATTACK,  name="Bite", combatant=self, to_hit=4, dmg_dice="1d10", dmg_bonus=2, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1, extra_dmg=[('1d4', DamageType.Cold)])
        self.add_ability(Action.CONIC_BREATH_WEAPON, recharge=5, dmg_dice='5d8', dmg_type=DamageType.Cold, saving_throw=SavingThrow.CON, dc=12, target_template=SpellStats.Target.CONE_15,  name="Cold Breath")
        self.add_ability(Reaction.REACTION_ATTACK,  name="Bite", combatant=self, to_hit=4, dmg_dice="1d10", dmg_bonus=2, dmg_type=DamageType.Piercing, attack_range=1, crit_range=1, extra_dmg=[('1d4', DamageType.Cold)])
