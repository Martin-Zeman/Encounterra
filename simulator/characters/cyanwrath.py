...
class Cyanwrath(Character):
    ...
    def __init__(self):
        cyanwrath_attacks = [Attack("Polearm", self, 7, "1d10", 4, "action", "Slashing", 2, [19, 20]),
                             Attack("Butt end of Polearm", self, 7, "1d4", 4, "bonus_action", "Bludgeoning", 2, [19, 20])]
        super().__init__("Cyanwrath", cyanwrath_attacks, 95, 17, 1, 30, ["Lightning"], num_attacks=2)

    def get_action(self, battle_map):
        chosen_action = None
        target = battle_map.get_nearest_enemy(self)
        for action in self.actions:
            if self.has_action and not action.is_bonus():
                ...
                if self.curr_num_attacks and self.multiattack_in_progress:
                    ...
                    chosen_action.set_target_character(target)
                    self.curr_num_attacks -= 1
                    logger.debug(f"{self.name} uses action {chosen_action.get_name()} against {target.get_name()}", extra={"team": self.team_name})
