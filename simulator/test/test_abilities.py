...
from simulator.test.fixtures import test_moon_druid_lvl_5, test_bugbear, test_giant_toad, teams, effect_tracker, battle_map, test_assassin_rogue,\
    test_ogre, test_goblin, test_brown_bear, test_dire_wolf, test_stone_giant, test_totem_barbarian, test_night_hag,\
    test_druid_lvl_1, test_fighter_lvl_1, test_battle_master_fighter_lvl_3, test_paladin_lvl_1, test_moon_druid_lvl_2, \
    test_young_green_dragon, test_skeleton, test_hobgoblin, test_green_dragon_wyrmling, test_moon_druid_lvl_3, \
    test_draconic_sorcerer_5lvl
...
def test_basic_wildshape(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
    ...
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_5, np.array([0, 0], dtype=np.int64))  # Have to set it for fireball placement
    ...
    test_moon_druid_lvl_5.concentration_effect = "FooBar"  # Must be non-None, This way we exclude all the concentration spells from the selection
...
def test_wildshape_with_concentration_spell(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_goblin):
    """
    We assert the basic functionality of the wildshape ability. The Druid must be able to wildshape and attack.
    """
    CustomLogger(logging.WARNING)
    ...
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_5, np.array([0, 0], dtype=np.int64))  # Have to set it for fireball placement
    battle_map.set_combatant_coordinates(test_goblin, np.array([4, 4], dtype=np.int64))  # Have to set it for fireball placement
    battle_map.build_adjacency_matrix()
    battle_map.set_effect_tracker(effect_tracker)
    combatants = [test_moon_druid_lvl_5, test_goblin]
    test_moon_druid_lvl_5.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_5, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_5.wildshape_factory[1])
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
...
def test_movement_before_wildshape_with_concentration_spell(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
    ...
    battle_map.set_effect_tracker(effect_tracker)
    battle_map.place_circular_element(np.array([0, 0], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([0, 1], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([0, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([0, 3], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([0, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([2, 0], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([2, 1], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([2, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([1, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([2, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([3, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([3, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    teams.add_combatant_to_team(test_moon_druid_lvl_5, Teams.Color.BLUE)  # For the log coloring...
    teams.add_combatant_to_team(test_bugbear, Teams.Color.RED)  # For the log coloring...
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_5, np.array([1, 0], dtype=np.int64))  # Have to set it for fireball placement
    battle_map.set_combatant_coordinates(test_bugbear, np.array([7, 6], dtype=np.int64))  # Have to set it for fireball placement
    battle_map.build_adjacency_matrix()
    battle_map.set_effect_tracker(effect_tracker)
    combatants = [test_moon_druid_lvl_5, test_bugbear]
    test_moon_druid_lvl_5.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_5, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_5.wildshape_factory[1])
    ...
    try:
        ...
        actoid9 = get_action(test_moon_druid_lvl_5)
        assert str(actoid9) == "None"
        test_moon_druid_lvl_5.new_turn()

        actoid10 = get_action(test_moon_druid_lvl_5)
        action_resolver.resolve_action(actoid10, test_moon_druid_lvl_5)
        actoid11 = get_action(test_moon_druid_lvl_5)
        action_resolver.resolve_action(actoid11, test_moon_druid_lvl_5)
        actoid12 = get_action(test_moon_druid_lvl_5)
        action_resolver.resolve_action(actoid12, test_moon_druid_lvl_5)
        actoid13 = get_action(test_moon_druid_lvl_5)
        action_resolver.resolve_action(actoid13, test_moon_druid_lvl_5)
        actoid14 = get_action(test_moon_druid_lvl_5)
        action_resolver.resolve_action(actoid14, test_moon_druid_lvl_5)
        actoid15 = get_action(test_moon_druid_lvl_5)
        action_resolver.resolve_action(actoid15, test_moon_druid_lvl_5)
...
def test_damage_knocks_out_of_wildshape(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
    ...
    try:
        ...
        action_resolver.resolve_action(actoid1, test_moon_druid_lvl_5)
        battle_map.move_combatant(test_bugbear, np.array([4, 4], dtype=np.int64))  # Move him back in case he got pushed away
        actoid2 = get_action(test_moon_druid_lvl_5)
        assert test_moon_druid_lvl_5.curr_hp == 43
        assert str(actoid2) == "Wildshape of Moon Druid 5th LVL (1) into Giant Toad"
        action_resolver.resolve_action(actoid2, test_moon_druid_lvl_5)
        ...
        test_moon_druid_lvl_5.get_current_form().receive_dmg(40, DamageType.Slashing)
        assert not is_affected_by(test_bugbear, Conditions.SWALLOWED)
        assert test_bugbear.is_swallowed == [False, None]
        if not test_moon_druid_lvl_5.concentration_effect:  # The damage could have interrupted it
            test_moon_druid_lvl_5.concentration_effect = dummy_effect  # Must be non-None, This way we exclude all the concentration spells from the selection
            battle_map.effect_tracker.add(dummy_effect)
        assert test_moon_druid_lvl_5.get_current_form() is test_moon_druid_lvl_5
        assert test_moon_druid_lvl_5.current_wildshape_form is None
        assert test_moon_druid_lvl_5.curr_hp == 42
        test_moon_druid_lvl_5.new_turn()
        actoid5 = get_action(test_moon_druid_lvl_5)
        ...
        assert test_moon_druid_lvl_5.get_current_form() is not test_moon_druid_lvl_5
        assert test_moon_druid_lvl_5.current_wildshape_form is not None
        assert test_moon_druid_lvl_5.get_current_form().curr_hp == 34
        test_moon_druid_lvl_5.get_current_form().receive_dmg(42, DamageType.Slashing)
        assert test_moon_druid_lvl_5.get_current_form() is test_moon_druid_lvl_5
        assert test_moon_druid_lvl_5.current_wildshape_form is None
        assert test_moon_druid_lvl_5.curr_hp == 34
        actoid6 = get_action(test_moon_druid_lvl_5)
...
def test_removed_from_battlemap_when_killed_by_carry_over_damage_in_wildshape(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
    ...
    test_moon_druid_lvl_5.concentration_effect = dummy_effect  # Must be non-None, This way we exclude all the concentration spells from the selection
    battle_map.effect_tracker.add(dummy_effect)
    test_moon_druid_lvl_5.curr_hp = 1
    test_moon_druid_lvl_5.spellslots.deplete_resource(ResourceDepletionLevel.FULLY_DEPLETED)  # To prevent the druid from casting Thunderwave or something first

    try:
        actoid1 = get_action(test_moon_druid_lvl_5)
        assert test_moon_druid_lvl_5.curr_hp == 1
        assert str(actoid1) == "Wildshape of Moon Druid 5th LVL (1) into Giant Toad"
        action_resolver.resolve_action(actoid1, test_moon_druid_lvl_5)
        assert test_moon_druid_lvl_5.get_current_form() is not test_moon_druid_lvl_5
        assert test_moon_druid_lvl_5.current_wildshape_form is not None
        assert test_moon_druid_lvl_5.get_current_form().curr_hp == 39
        wildshaped_moon_druid = test_moon_druid_lvl_5.get_current_form()
        ...
        assert not test_moon_druid_lvl_5.is_alive()
        assert test_moon_druid_lvl_5.get_current_form() is test_moon_druid_lvl_5
        assert test_moon_druid_lvl_5 not in battle_map.combatant_coordinate_cache.keys()
    except Exception as e:
        assert False, f"Raised an exception {e}"


def test_others_can_attack_wildshape(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
...
def test_wilshape_get_eligible_coords(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
    ...
    teams.add_combatant_to_team(test_moon_druid_lvl_5, Teams.Color.BLUE)  # For the log coloring...
    teams.add_combatant_to_team(test_bugbear, Teams.Color.RED)  # For the log coloring...
    battle_map.place_circular_element(np.array([1, 13], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([5, 13], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([10, 13], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([13, 13], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([13, 9], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([7, 9], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([4, 9], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([1, 9], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([11, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=3)
    battle_map.place_circular_element(np.array([3, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=4)
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_5, np.array([10, 10], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_bugbear, np.array([5, 11], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    distances, shortest_paths = battle_map.calc_dijkstra(test_moon_druid_lvl_5)
    battle_map.set_effect_tracker(effect_tracker)
    ...
    wsf = WildshapeFactory(test_moon_druid_lvl_5, BonusAction.MOON_WILDSHAPE)
    ws = wsf.create(GiantConstrictorSnake)
    coords, _ = ws.get_eligible_coords(distances, shortest_paths)
    assert coords == [(9, 9)]


def test_wilshape_copy_two_druids(battle_map, teams, effect_tracker, test_moon_druid_lvl_5, test_bugbear):
    """
    We make sure there's a clearing in the terrain which the giant form fits into. It starts at root coordinate [9, 8].
    """
    test_moon_druid_lvl_5_2 = copy.deepcopy(test_moon_druid_lvl_5)
    battle_map.set_effect_tracker(effect_tracker)
    teams.add_combatant_to_team(test_moon_druid_lvl_5, Teams.Color.BLUE)  # For the log coloring...
    teams.add_combatant_to_team(test_bugbear, Teams.Color.RED)  # For the log coloring...
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_5, np.array([0, 0], dtype=np.int64))  # Have to set it for fireball placement
    ...
    combatants = [test_moon_druid_lvl_5, test_bugbear]
    test_moon_druid_lvl_5.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_5, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_5.wildshape_factory[1])
    test_moon_druid_lvl_5_2.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_5_2, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_5_2.wildshape_factory[1])
    assert test_moon_druid_lvl_5.available_wildshape_forms[0] is not test_moon_druid_lvl_5_2.available_wildshape_forms[0]
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
...
def test_bite_and_swallow(battle_map, teams, effect_tracker, test_giant_toad, test_bugbear):
    ...
    teams.add_combatant_to_team(test_bugbear, Teams.Color.RED)  # For the log coloring...
    battle_map.set_combatant_coordinates(test_giant_toad, np.array([2, 2], dtype=np.int64))  # Have to set it for fireball placement
    battle_map.set_combatant_coordinates(test_bugbear, np.array([4, 4], dtype=np.int64))  # Have to set it for fireball placement
    battle_map.build_adjacency_matrix()
    battle_map.set_effect_tracker(effect_tracker)
...
@pytest.mark.flaky(reruns=3)
def test_cannot_wildshape_restrained_in_confined_space(battle_map, teams, effect_tracker, test_giant_toad, test_moon_druid_lvl_5):
    """
    ...
    CustomLogger(logging.WARNING)
    battle_map.set_effect_tracker(effect_tracker)
    combatants = [test_moon_druid_lvl_5, test_giant_toad]
    test_moon_druid_lvl_5.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_5, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_5.wildshape_factory[1])
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
    ...
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_5, np.array([1, 14], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_giant_toad, np.array([0, 12], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    test_moon_druid_lvl_5.ac = 0  # Make sure the bite hits (expect for nat 1)
    for af in test_giant_toad.action_factories:
        try:
            af[1].to_hit = 100
        except AttributeError:
            pass
    test_moon_druid_lvl_5.athletics = -20  # Make sure it can't break the grapple
    test_moon_druid_lvl_5.acrobatics = -20  # Make sure it can't break the grapple
    test_moon_druid_lvl_5.spellslots.deplete_resource(ResourceDepletionLevel.FULLY_DEPLETED)  # To prevent the druid from casting instead

    try:
        actoid1 = get_action(test_giant_toad)
        action_resolver.resolve_action(actoid1, test_giant_toad)
        if is_affected_by(test_moon_druid_lvl_5, Conditions.GRAPPLED):
            actoid2 = get_action(test_moon_druid_lvl_5)
            action_resolver.resolve_action(actoid2, test_moon_druid_lvl_5)
            if is_affected_by(test_moon_druid_lvl_5, Conditions.GRAPPLED):  # Still grappled
                actoid3 = get_action(test_moon_druid_lvl_5)
...
def test_cunning_hide_geometry(battle_map, teams, effect_tracker, test_assassin_rogue, test_bugbear, test_ogre, test_goblin):
    ...
    CustomLogger(logging.WARNING)
    battle_map.set_effect_tracker(effect_tracker)
    battle_map.place_circular_element(np.array([6, 8], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([8, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([2, 11], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([11, 12], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    ...
    battle_map.set_combatant_coordinates(test_assassin_rogue, np.array([1, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_bugbear, np.array([12, 8], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_ogre, np.array([2, 1], dtype=np.int64))
    ...
    eligible_coords, _ = hide.get_eligible_coords(None, None)
...
def test_cunning_hide_and_sneak_attack(battle_map, teams, effect_tracker, test_assassin_rogue, test_bugbear, test_ogre, test_goblin, test_brown_bear):
    ...
    battle_map.set_combatant_coordinates(test_ogre, np.array([2, 1], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([5, 11], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_brown_bear, np.array([13, 0], dtype=np.int64))
    battle_map.build_adjacency_matrix()
...
def test_cunning_adjacent_enemy_hide_sneak_attack_2(battle_map, teams, effect_tracker, test_assassin_rogue, test_bugbear, test_ogre, test_goblin):
    ...
    battle_map.place_circular_element(np.array([6, 8], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([8, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([2, 9], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([11, 12], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    combatants = [test_assassin_rogue, test_bugbear, test_ogre, test_goblin]
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
    teams.add_combatant_to_team(test_assassin_rogue, Teams.Color.BLUE)
...
def test_cunning_adjacent_enemy_hide_sneak_attack_in_melee(battle_map, teams, effect_tracker, test_stone_giant, test_assassin_rogue, test_dire_wolf):
    ...
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
    battle_map.place_circular_element(np.array([2, 13], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([11, 10], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([10, 10], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([11, 5], dtype=np.int64), Terrain.DIFFICULT_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([5, 5], dtype=np.int64), Terrain.DIFFICULT_TERRAIN, radius=0)
    ...
    battle_map.set_combatant_coordinates(test_stone_giant, np.array([9, 11], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_assassin_rogue, np.array([12, 10], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_dire_wolf, np.array([7, 10], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    for combatant in combatants:
        combatant.curr_init = 0  # To prevent assassinate from crashing
...
def test_rogue_cunning_disengage(battle_map, teams, effect_tracker, test_assassin_rogue, test_bugbear, test_ogre, test_goblin):
    ...
    CustomLogger(logging.WARNING)
    battle_map.set_effect_tracker(effect_tracker)
    battle_map.place_circular_element(np.array([4, 5], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    combatants = [test_assassin_rogue, test_bugbear, test_ogre, test_goblin]
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
    teams.add_combatant_to_team(test_assassin_rogue, Teams.Color.BLUE)
    teams.add_combatant_to_team(test_bugbear, Teams.Color.RED)
    teams.add_combatant_to_team(test_ogre, Teams.Color.RED)
    teams.add_combatant_to_team(test_goblin, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_assassin_rogue, np.array([2, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_bugbear, np.array([3, 2], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_ogre, np.array([1, 1], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([1, 3], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    test_assassin_rogue.stealth = 20  # Making sure the hide always works
    for combatant in combatants:
        combatant.curr_init = 0  # To prevent assassinate from crashing
...
def test_rogue_cunning_dash(battle_map, teams, effect_tracker, test_assassin_rogue, test_bugbear, test_ogre, test_goblin):
    ...
    teams.add_combatant_to_team(test_ogre, Teams.Color.RED)
    teams.add_combatant_to_team(test_goblin, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_assassin_rogue, np.array([7, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_bugbear, np.array([6, 1], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_ogre, np.array([7, 0], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([9, 1], dtype=np.int64))
    battle_map.build_adjacency_matrix()
...
def test_ray_of_enfeeblement(battle_map, teams, effect_tracker, test_totem_barbarian, test_night_hag):
    ...
    battle_map.set_combatant_coordinates(test_totem_barbarian, np.array([7, 4], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_night_hag, np.array([7, 5], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    FULL_HP = 1000
...
def test_sleep_loss_of_concentration(battle_map, teams, effect_tracker, test_goblin, test_night_hag):
    ...
    teams.add_combatant_to_team(test_night_hag, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_goblin, np.array([4, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin_2, np.array([5, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_night_hag, np.array([9, 5], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    ...
    try:
        ...
        sleep = sleep_factory.create(np.array([4, 5], dtype=np.int64))
...
def test_faerie_fire(battle_map, teams, effect_tracker, test_druid_lvl_1, test_fighter_lvl_1):
    ...
    CustomLogger(logging.WARNING)
    battle_map.set_effect_tracker(effect_tracker)
    battle_map.place_circular_element(np.array([6, 3], dtype=np.int64), Terrain.DIFFICULT_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([7, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([7, 7], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    ...
    battle_map.set_combatant_coordinates(test_druid_lvl_1, np.array([0, 10], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_fighter_lvl_1, np.array([13, 8], dtype=np.int64))
    battle_map.build_adjacency_matrix()
...
def test_menacing_attack_shares_ammo(battle_map, teams, effect_tracker, test_druid_lvl_1, test_battle_master_fighter_lvl_3):
    ...
    teams.add_combatant_to_team(test_druid_lvl_1, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_druid_lvl_1, np.array([5, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_battle_master_fighter_lvl_3, np.array([7, 5], dtype=np.int64))
    action_resolver = ActionResolver(combatants, teams, effect_tracker)
...
def test_lay_on_hands(battle_map, teams, effect_tracker, test_paladin_lvl_1, test_druid_lvl_1, test_battle_master_fighter_lvl_3):
    ...
    battle_map.set_combatant_coordinates(test_paladin_lvl_1, np.array([5, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_druid_lvl_1, np.array([7, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_battle_master_fighter_lvl_3, np.array([14, 5], dtype=np.int64))
...
def test_conic_breath_weapon_placement(battle_map, teams, effect_tracker, test_young_green_dragon, test_skeleton, test_bugbear, test_hobgoblin):
    ...
    battle_map.place_circular_element(np.array([8, 7], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([14, 8], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=1)
    battle_map.place_circular_element(np.array([11, 4], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    battle_map.place_circular_element(np.array([12, 2], dtype=np.int64), Terrain.IMPASSABLE_TERRAIN, radius=0)
    ...
    battle_map.set_combatant_coordinates(test_young_green_dragon, np.array([10, 9], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_skeleton, np.array([12, 8], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_bugbear, np.array([11, 8], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_bugbear_2, np.array([8, 11], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_hobgoblin, np.array([1, 5], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_hobgoblin_2, np.array([6, 0], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    ...
    eligible_coords, _ = all_breaths[failing_instance_index].get_eligible_coords(distances, shortest_paths)
...
def test_conic_breath_weapon_placement_into_an_obstacle(battle_map, teams, effect_tracker, test_green_dragon_wyrmling, test_skeleton):
    ...
    teams.add_combatant_to_team(test_skeleton, Teams.Color.BLUE)
    battle_map.set_combatant_coordinates(test_green_dragon_wyrmling, np.array([10, 9], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_skeleton, np.array([12, 8], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    ...
    battle_map.clear_caches()
    eligible_coords, _ = breath.get_eligible_coords(distances, shortest_paths)
    assert not eligible_coords
...
def test_second_wind_used(battle_map, teams, effect_tracker, test_fighter_lvl_1, test_skeleton, test_goblin):
    ...
    teams.add_combatant_to_team(test_goblin, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_fighter_lvl_1, np.array([3, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_skeleton, np.array([5, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([2, 3], dtype=np.int64))
    battle_map.build_adjacency_matrix()
    battle_map.set_effect_tracker(effect_tracker)
...
def test_pounce_is_straight_short_path():
    ...
    assert nf.is_path_straight(path, 2) is True
    assert nf.is_path_straight(path, 1) is False
...
def test_pounce_is_straight_length_exceeds_path():
    ...
    assert nf.is_path_straight(path, 4) is False


def test_pounce_is_straight_horizontal_path():
    path = np.array([[0, 0], [1, 0], [2, 0], [3, 0]])
    assert nf.is_path_straight(path, 4) is True
...
def test_pounce_is_straight_diagonal_path():
    ...
    assert nf.is_path_straight(path, 5) is True
    assert nf.is_path_straight(path, 6) is False
    assert nf.is_path_straight(path, 3) is True


def test_binning_01(battle_map, teams, effect_tracker, test_moon_druid_lvl_2, test_skeleton, test_hobgoblin, test_goblin):
    """
    Investigating Faerie Fire
    """
    CustomLogger(logging.INFO)
    battle_map.set_effect_tracker(effect_tracker)
    test_moon_druid_lvl_2.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_2, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_2.wildshape_factory[1])
    teams.add_combatant_to_team(test_moon_druid_lvl_2, Teams.Color.BLUE)
    teams.add_combatant_to_team(test_skeleton, Teams.Color.RED)
    teams.add_combatant_to_team(test_hobgoblin, Teams.Color.RED)
    teams.add_combatant_to_team(test_goblin, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_2, np.array([3, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_skeleton, np.array([7, 11], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_hobgoblin, np.array([6, 12], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([12, 4], dtype=np.int64))

    battle_map.build_adjacency_matrix()

    try:
        actoid1 = get_action(test_moon_druid_lvl_2)
    except Exception as e:
        assert False, f"Raised an exception {e}"

def test_binning_02(battle_map, teams, effect_tracker, test_moon_druid_lvl_3, test_skeleton, test_hobgoblin, test_goblin):
    """
    Investigating Flaming Sphere
    """
    CustomLogger(logging.INFO)
    battle_map.set_effect_tracker(effect_tracker)
    test_moon_druid_lvl_3.available_wildshape_forms = preallocate_wildshape_forms(test_moon_druid_lvl_3, BonusAction.MOON_WILDSHAPE, test_moon_druid_lvl_3.wildshape_factory[1])
    teams.add_combatant_to_team(test_moon_druid_lvl_3, Teams.Color.BLUE)
    teams.add_combatant_to_team(test_skeleton, Teams.Color.RED)
    teams.add_combatant_to_team(test_hobgoblin, Teams.Color.RED)
    teams.add_combatant_to_team(test_goblin, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_3, np.array([3, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_skeleton, np.array([7, 11], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_hobgoblin, np.array([6, 12], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([12, 4], dtype=np.int64))
    ...
    try:
        actoid1 = get_action(test_moon_druid_lvl_3)
    except Exception as e:
        assert False, f"Raised an exception {e}"


def test_binning_03(battle_map, teams, effect_tracker, test_draconic_sorcerer_5lvl, test_moon_druid_lvl_3, test_skeleton, test_hobgoblin, test_goblin):
    """
    Investigating Haste
    """
    CustomLogger(logging.INFO)
    ...
    teams.add_combatant_to_team(test_hobgoblin, Teams.Color.RED)
    teams.add_combatant_to_team(test_goblin, Teams.Color.RED)
    battle_map.set_combatant_coordinates(test_draconic_sorcerer_5lvl, np.array([3, 9], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_moon_druid_lvl_3, np.array([3, 3], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_skeleton, np.array([7, 11], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_hobgoblin, np.array([6, 12], dtype=np.int64))
    battle_map.set_combatant_coordinates(test_goblin, np.array([12, 4], dtype=np.int64))
