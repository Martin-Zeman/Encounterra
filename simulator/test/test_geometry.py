...
def test_cone_15_feet():
    coords = get_affected_by_cone(origin=np.array([2, 0], dtype=np.int64), angle_deg=45, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_15],
                                  grid_size=15)
    expected_coords = {(3, 1), (4, 1), (3, 2), (4, 2)}
    assert coords == expected_coords

    coords = get_affected_by_cone(origin=np.array([2, 0], dtype=np.int64), angle_deg=29, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_15],
                                  grid_size=15)
    expected_coords = {(2, 1), (3, 1), (2, 2), (3, 2), (4, 2)}
    assert coords == expected_coords

    coords = get_affected_by_cone(origin=np.array([2, 0], dtype=np.int64), angle_deg=0, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_15],
                                  grid_size=15)
    expected_coords = {(2, 1), (2, 2), (1, 2), (3, 2)}
    assert coords == expected_coords

    coords = get_affected_by_cone(origin=np.array([2, 0], dtype=np.int64), angle_deg=30, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_15],
                                  grid_size=15)
...
def test_cone_30_feet():
    coords = get_affected_by_cone(origin=np.array([4, 7], dtype=np.int64), angle_deg=180, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_30], grid_size=15)
    expected_coords = {(2, 2), (3, 2), (4, 2), (5, 2), (6, 2), (3, 2), (2, 3), (3, 3), (4, 3), (5, 3), (6, 3), (3, 4), (4, 4), (5, 4),
                       (3, 5), (4, 5), (5, 5), (4, 6)}
    assert coords == expected_coords

    coords = get_affected_by_cone(origin=np.array([1, 4], dtype=np.int64), angle_deg=90, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_30], grid_size=15)
    expected_coords = {(2, 4), (3, 3), (3, 4), (3, 5), (4, 3), (4, 4), (4, 5), (5, 2), (5, 3), (5, 4), (5, 5), (5, 6), (6, 2), (6, 3),
                       (6, 4), (6, 5), (6, 6)}
    assert coords == expected_coords

    coords = get_affected_by_cone(origin=np.array([7, 4], dtype=np.int64), angle_deg=270, radius=SpellStats.TRANSLATE_CONE[SpellStats.Target.CONE_30], grid_size=15)
...
def test_line_vertical():
    origin = np.array([3, 3], dtype=np.int64)
    angle_deg = 0
    length = 5
    width = 1
    grid_size = 15
...
def test_line_diagonal():
    origin = np.array([2, 2], dtype=np.int64)
    angle_deg = 45
    length = 4
    width = 2
    grid_size = 15
...
def test_line_origin_opposite_edge():
    origin = np.array([12, 7], dtype=np.int64)
    angle_deg = 90
    length = 5
    width = 1
    grid_size = 15
...
def test_angle_between_vectors():
    assert nf.angle_between_vectors_int(np.array([0, 1]), np.array([1, 0])) == pytest.approx(90.0, 0.0001)
    assert nf.angle_between_vectors(np.array([0, 1.]), np.array([1., 0])) == pytest.approx(90.0, 0.0001)
    assert nf.angle_between_vectors_int(np.array([0, 1]), np.array([1, -1])) == pytest.approx(135.0, 0.0001)
    assert nf.angle_between_vectors(np.array([0, 1.]), np.array([1., -1.])) == pytest.approx(135.0, 0.0001)
    assert nf.angle_between_vectors_int(np.array([0, 1]), np.array([0, -1])) == pytest.approx(180.0, 0.0001)
    assert nf.angle_between_vectors(np.array([0, 1.]), np.array([0, -1.])) == pytest.approx(180.0, 0.0001)
    assert nf.angle_between_vectors_int(np.array([0, 2]), np.array([-1, 2])) == pytest.approx(26.5650, 0.0001)
    assert nf.angle_between_vectors(np.array([0, 2.]), np.array([-1., 2.])) == pytest.approx(26.5650, 0.0001)
    assert nf.angle_between_vectors(np.array([1, 0.5]), np.array([1.5, -1])) == pytest.approx(60.2551, 0.0001)
    assert nf.angle_between_vectors_int(np.array([6, 4]), np.array([6, 4])) == pytest.approx(0, 0.0001)
    assert nf.angle_between_vectors(np.array([6., 4.]), np.array([6., 4.])) == pytest.approx(0, 0.0001)
    assert nf.angle_between_vectors_int(np.array([0, 4]), np.array([4, 4])) == pytest.approx(45.0, 0.0001)
    assert nf.angle_between_vectors(np.array([0, 4.]), np.array([4., 4.])) == pytest.approx(45.0, 0.0001)
...
def test_find_fov_vectors(test_stone_giant, test_ogre, test_bugbear):
    ...
    outlines = find_fov_vectors(Coords(np.array([3, 7]), test_bugbear.size.value), Coords(np.array([6, 6]), test_stone_giant.size.value))
    ...
    outlines = find_fov_vectors(Coords(np.array([6, 6]), test_stone_giant.size.value), Coords(np.array([3, 7]), test_bugbear.size.value))
    assert len(outlines) == 2
    assert any([np.array_equal(np.array([-3.5, 0.5]) / np.linalg.norm(np.array([-3.5, 0.5])), point) for point in outlines])
    assert any([np.array_equal(np.array([-3.5, -0.5]) / np.linalg.norm(np.array([-3.5, 0.5])), point) for point in outlines])
    # At a slight angle
    outlines = find_fov_vectors(Coords(np.array([0, 0]), test_stone_giant.size.value), Coords(np.array([5, 2]), test_ogre.size.value))
    assert len(outlines) == 2
    ...
    outlines = find_fov_vectors(Coords(np.array([5, 2]), test_bugbear.size.value), Coords(np.array([6, 6]), test_stone_giant.size.value))
    assert len(outlines) == 2
    ...
    outlines = find_fov_vectors(Coords(np.array([6, 2]), test_bugbear.size.value), Coords(np.array([6, 6]), test_stone_giant.size.value))
    assert len(outlines) == 2
...
def test_get_bounding_box():
    ...
    coord1 = Coords(np.array([1, 1]), Size.MEDIUM.value)
    coord2 = Coords(np.array([3, 3]), Size.MEDIUM.value)
    bottom_left, top_right = nf.get_bounding_box(coord1.get(), coord2.get())
    ...
    coord1 = Coords(np.array([0, 0]), Size.SMALL.value)
    coord2 = Coords(np.array([4, 4]), Size.LARGE.value)
    bottom_left, top_right = nf.get_bounding_box(coord1.get(), coord2.get())
    ...
    coord1 = Coords(np.array([2, 2]), Size.HUGE.value)
    coord2 = Coords(np.array([3, 3]), Size.GARGANTUAN.value)
    bottom_left, top_right = nf.get_bounding_box(coord1.get(), coord2.get())
    ...
    coord1 = Coords(np.array([0, 0]), Size.TINY.value)
    coord2 = Coords(np.array([0, 0]), Size.TINY.value)
    bottom_left, top_right = nf.get_bounding_box(coord1.get(), coord2.get())
    assert np.array_equal(bottom_left, np.array([0, 0]))
    assert np.array_equal(top_right, np.array([0, 0]))

    # Test case 5: HUGE and LARGE
    coords1 = Coords(np.array([1, 11]), Size.HUGE.value)
    coords2 = Coords(np.array([9, 13]), Size.LARGE.value)
    bottom_left, top_right = nf.get_bounding_box(coords1.get(), coords2.get())
