...
def resolve_dmg_saving_throw(ability, dmg, target, half_on_success=True, is_spell_effect=False):
    ...
    if rolled == 1:
        ...
        for bonus_die in target.saving_throws_dice_mod[ability.factory.saving_throw]:
            bonus_dice_roll = nf.roll_dice(bonus_die)
...
class ActionResolver:
    ...
    def resolve_ranged_spell_attack(self, caster, spell, target):
        ...
        if rolled + spell.factory.to_hit >= target.ac:
            dmg = multiplier * nf.roll_dice_multi(spell.factory.dmg_dice)
    ...
    def resolve_attack(self, attack, target, attacker):  # TODO remove combatant from attack and have it as a separate parameter
        ...
        if FactoryFlags.IS_PRECISION in attack.factory.flags and rolled + attack.factory.to_hit < (
                target.ac + target.one_time_ac_bonus) <= rolled + attack.factory.to_hit + nf.avg_roll(
                attack.factory.to_hit_bonus_die) and attacker.resources[Passive.BATTLE_MASTER_MANEUVERS].has_resource():
            to_hit_maneuver_bonus = nf.roll_dice(attack.factory.to_hit_bonus_die)
        ...
        if rolled + attack.factory.to_hit + to_hit_maneuver_bonus >= (target.ac + target.one_time_ac_bonus):  # Potentially missing or hitting this time
            ...
            if FactoryFlags.TWO_HANDED in attack.factory.flags and attacker.has_passive(Passive.GREAT_WEAPON_FIGHTING):
                ...
                if len(dice) > 1:  # Shouldn't be necessary but njit gives me trouble otherwise
                    dmg_dice_sum += nf.roll_dice_multi(dice[1:])  # Only the weapon dice can be rerolled
            else:
                dmg_dice_sum = nf.roll_dice_multi(dice)
            # logger.info(f"Rolled {dmg_dice_sum} on the dmg dice", extra={"team": self.teams.get_team(attacker)})
            extra_dmg = [(multiplier * nf.roll_dice(e[0]), e[1]) for e in attack.factory.extra_dmg]
    ...
    def resolve_by_actoid_flags(self, actoid, combatant):
        ...
        match actoid.factory.action_type:
            ...
            case Action.FIREBALL | BonusAction.QUICKENED_FIREBALL:
                ...
                dmg = nf.roll_dice_multi(actoid.factory.dmg_dice)
            ...
            case BonusAction.FLAMING_SPHERE_RAM:
                ...
                _, shortest_paths = nf.dijkstra(actoid.factory.action_enabler_effect.origin, battle_map.size, adj, mask)
                path = battle_map.get_effect_path_to_coord(actoid.factory.action_enabler_effect.origin, actoid.coord, shortest_paths)
                if path.shape[0] != 0 and len(path) <= FlamingSphereRamFactory.RANGE + 1:
                    dmg = nf.roll_dice_multi(actoid.factory.dmg_dice)
            ...
            case Action.MAGIC_MISSILE | BonusAction.QUICKENED_MAGIC_MISSILE:
                logger.info(f"{combatant} casts {actoid}")
                dmg_dice_sum = nf.roll_dice_multi(actoid.factory.dmg_dice) + actoid.factory.dmg_bonus
            ...
            case BonusAction.SECOND_WIND:
                heal_hp = nf.roll_dice((1, 10)) + combatant.level
            ...
            case BonusAction.HEALING_WORD | Action.CURE_WOUNDS:
                logger.info(f"{combatant} casts {actoid}")
                heal_hp = nf.roll_dice(actoid.factory.heal_dice) + actoid.factory.mod
