...
class HideFactory(ThreatModifierFactory):
    ...
    def create_all(self, previous_action_in_dag=None):
        if get_swallower(self.combatant):
            return []
        battle_map = Map.get()
        return [Hide(e, self) for e in Map.get().get_non_swallowed_enemies(self.combatant) if not battle_map.effect_tracker.is_combatant_hidden_from(self.combatant, e)]
...
class Hide(AttackThreatModifier, CombatantEffect):
    ...
    def calculate_threat_for_attack(self, combatant, attack, *args, **kwargs):
        ...
        if getattr(attack, "target", None) is self.target and (FactoryFlags.IS_ATTACK_LIKE in attack.factory.flags or
                                                             (FactoryFlags.USES_DEX in attack.factory.flags or
