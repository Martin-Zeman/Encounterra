...
class Hide(Actoid, CombatantEffect, AttackThreatModifier):
    ...
    def calculate_threat_for_attack(self, combatant, attack, *args, **kwargs):
        ...
        if battle_map.is_ally_adjacent_to_target(combatant, attack.target):
            return threat_acc  # Sneak Attack condition already made, no extra benefit
        if isinstance(attack.on_hit, OnHitSneakAttack):
            threat_acc += attack.on_hit.calculate_threat(combatant, attack.target)
        return threat_acc
