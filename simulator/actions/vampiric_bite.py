...
class VampiricBiteFactory(DirectThreatFactory):
    ...
    def get_eligible_targets(self):
        ...
        if swallower:
            return []
        return [e for e in Map.get().get_enemies(self.combatant) if not e.is_affected_by(Conditions.SWALLOWED) and e.is_affected_by_any(Conditions.GRAPPLED, Conditions.INCAPACITATED, Conditions.RESTRAINED)]
    ...
    def calculate_threat_to_target(self, target, **kwargs):
        ...
        if not consider_dist or Map.get().get_hop_distance_combatants(self.combatant, target) <= self.range:
            ...
            return acc * 2.2 # Because of the life drain and accounting for the reduced maximum
        return 0
    ...
    def calculate_max_threat(self):
        ...
        return max([self.calculate_threat_to_target(t) for t in targets]) * 2.2 # Because of the life drain and accounting for the reduced maximum


class VampiricBite(Actoid, DirectThreat):
